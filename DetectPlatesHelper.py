...
def detectPlatesInScene(imgOriginalScene):
    ...
    if Main.showSteps == True: # show steps
        cv2.imshow("GRAY SCALE IMAGE", imgGrayscaleScene)
        cv2.imshow("THRESHOLD IMAGE", imgThreshScene)
    # end if # show steps
    ...
    listOfListsOfMatchingCharsInScene = DetectCharactersHelper.findListOfListsOfMatchingChars(listOfPossibleCharsInScene)
...
def findPossibleCharsInScene(imgThresh):
    ...
    for i in range(0, len(contours)):                       # for each contour
        ...
        if DetectCharactersHelper.checkIfPossibleChar(possibleChar):                   # if contour is a possible char, note this does not compare to other chars (yet) . . .
...
def extractPlate(imgOriginal, listOfMatchingChars):
    ...
    fltHypotenuse = DetectCharactersHelper.distanceBetweenChars(listOfMatchingChars[0], listOfMatchingChars[len(listOfMatchingChars) - 1])
