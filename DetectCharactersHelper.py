...
def loadDataAndTrain():
    ...
    except:                                                                                 # if file couldnt be opened
        print("error, unable to open classifications.txt, exiting program\n")  # show error message
        np.os.system("pause")   #pause to check the error code
    ...
    kNearest.setDefaultK(1)                                                             # set default K-nearest neighbour to 1
    kNearest.train(npaFlattenedImages, cv2.ml.ROW_SAMPLE, npaClassifications)           # train k-nearest neighbors  object
...
def detectCharsInPlates(listOfPossiblePlates):
    ...
    if len(listOfPossiblePlates) == 0:          #is empty
    ...
    for possiblePlate in listOfPossiblePlates:
        ...
        possiblePlate.imageGrayscale, possiblePlate.imgThresh = Preprocess.preprocess(possiblePlate.imgPlate)     # preprocess to get grayscale and threshold images

        if Main.showSteps == True: # show steps ###################################################
            cv2.imshow("Image Plate", possiblePlate.imgPlate)
            cv2.imshow("Image GrayScale", possiblePlate.imageGrayscale)
            cv2.imshow("Image Threshold", possiblePlate.imgThresh)
        # end if # show steps #####################################################################

                # increases the size of picture and zooms to plate for character detection
        possiblePlate.imgThresh = cv2.resize(possiblePlate.imgThresh, (0, 0), fx = 1.5, fy = 1.6)

                # threshold again to eliminate any gray areas
        thresholdValue, possiblePlate.imgThresh = cv2.threshold(possiblePlate.imgThresh, 0.0, 255.0, cv2.THRESH_BINARY | cv2.THRESH_OTSU)

        if Main.showSteps == True : # show steps ###################################################
            cv2.imshow("Threshold image part 1", possiblePlate.imgThresh)
        # end if # show steps #####################################################################


                # this function first finds all contours, then only includes contours that could be chars (without comparison to other chars yet)
        listOfPossibleCharsInPlate = findPossibleCharsInPlate(possiblePlate.imageGrayscale, possiblePlate.imgThresh)
        ...
        if (len(listOfListsOfMatchingCharsInPlate) == 0):			# if no groups of matching chars were found in the plate
            ...
            if Main.showSteps == True: # show steps in main.py should be true
